(function () {
  ...
  const AUDIO = {
    paths: {
      ...
      tile:      'sfx/tile-select.wav',
      invalid:   'sfx/xTile.wav',
      walk:      'sfx/walk.wav',
      hitPlayer: 'DeadGolem1.wav',
      hitEnemy:  'sfx/SlimeDead1.wav',
      heal:      'sfx/heal-cast.wav',
      lose:      'bgm/lose.wav',
      lobby:     'bgm/lobby-bgm(1).mp3',
      ingame:    'bgm/ingame-bgm(1).mp3',
      win:       'bgm/fanfare-bgm(1).mp3'
    },
    enabled: false,
    bgm: null,
    walkPool: [],
  ...
  function playSfx(path, volume = 0.72, restart = true) {
    if (!AUDIO.enabled) return null;
    const a = makeAudio(path, false);
    a.volume = volume;
  ...
  function playUiClick() {
    const now = Date.now();
    if (now < AUDIO.uiThrottleUntil) return;
    AUDIO.uiThrottleUntil = now + 70;
    playSfx(AUDIO.paths.uiClick, 0.42);
  }

  async function playDiceLayer(duration = 900) {
    if (!AUDIO.enabled) return;
    const pulses = Math.max(4, Math.floor(duration / 180));
    for (let i = 0; i < pulses; i++) {
      playSfx(AUDIO.paths.uiClick, 0.16);
      await sleep(Math.max(80, Math.floor(duration / pulses)));
    }
    playSfx(AUDIO.paths.tile, 0.24);
  }

  function playWalkStep() {
    playSfx(AUDIO.paths.walk, 0.34);
  }

  function playPlayerHit() {
    playSfx(AUDIO.paths.hitPlayer, 0.82);
  }

  function playEnemyHit() {
    playSfx(AUDIO.paths.hitEnemy, 0.82);
  }

  function playHealSfx() {
    playSfx(AUDIO.paths.heal, 0.7);
  }
  ...
  function playBgm(path, volume = 0.45, loop = true) {
    if (!AUDIO.enabled) return;
    if (AUDIO.bgm && AUDIO.bgm.src && AUDIO.bgm.src.includes(path)) return;
    stopBgm();
    const a = makeAudio(path, loop);
    a.volume = volume;
    AUDIO.bgm = a;
    const p = a.play();
    if (p && typeof p.catch === 'function') p.catch(() => {});
  ...
  function playLoseSting() {
    stopBgm();
    playSfx(AUDIO.paths.lose, 0.86);
  }

  function playWinBgm() {
    playBgm(AUDIO.paths.win, 0.48, true);
  }

  function playLobbyBgm() {
    playBgm(AUDIO.paths.lobby, 0.42, true);
  }

  window.GameAudio = {
    unlockAudio,
    uiClick: playUiClick,
    tileValid: () => playSfx(AUDIO.paths.tile, 0.38),
    tileInvalid: () => playSfx(AUDIO.paths.invalid, 0.52),
    walkStep: playWalkStep,
    playerHit: playPlayerHit,
    enemyHit: playEnemyHit,
    heal: playHealSfx,
    diceRoll: playDiceLayer,
    playLobbyBgm,
    playIngameBgm: () => playBgm(AUDIO.paths.ingame, 0.44, true),
    playWinBgm,
    playLoseSting,
    stopBgm
  };
  ...
  function initGame() {
    ...
    hideModals();
    clearLog();
    window.GameAudio?.playIngameBgm();
  ...
  async function onRollClick() {
    ...
    const diceAudio = window.GameAudio?.diceRoll?.(900);
    await ThreeScene.animateDiceRoll(val);
    await Promise.resolve(diceAudio);
    showDiceResult(val);
  ...
  function onTileClick(col, row) {
    if (state.phase !== 'pathPlanning' || state.busy) { window.GameAudio?.tileInvalid?.(); return; }
    ...
    if (stepsUsed > 0 && tip.col === col && tip.row === row) {
      path.pop();
      window.GameAudio?.tileValid?.();
      refreshPathUI();
      return;
    }

    // Must be adjacent to tip
    if (Math.abs(col - tip.col) + Math.abs(row - tip.row) !== 1) { window.GameAudio?.tileInvalid?.(); return; }

    // No steps left
    if (stepsLeft <= 0) { window.GameAudio?.tileInvalid?.(); return; }

    // Must be walkable (not blocked, not enemy tile)
    if (!isWalkable(col, row, 'player')) { window.GameAudio?.tileInvalid?.(); return; }
  ...
  async function onAttack() {
    ...
    const attackAnim = ThreeScene.animateAttack(true);
    await sleep(140);
  ...
  async function doBotTurn() {
    ...
    if (isAdjacent(state.bot, state.player)) {
      ...
      if (botShouldHeal()) {
        ...
        const attackAnim = ThreeScene.animateAttack(false);
        await sleep(140);
  ...
  function wireButtons() {
    ...
    const maybeHook = (id, fn) => {
      const el = document.getElementById(id);
      if (el) el.addEventListener('click', fn);
    };
    maybeHook('gw-menu', () => window.GameAudio?.uiClick());
    maybeHook('gw-endturn', () => window.GameAudio?.uiClick());
    maybeHook('gw-nextround', () => window.GameAudio?.uiClick());
    maybeHook('gw-lobby', () => {
      window.GameAudio?.uiClick();
      window.GameAudio?.playLobbyBgm?.();
  ...
  async function bootstrap() {
    document.addEventListener('pointerdown', () => window.GameAudio?.unlockAudio?.(), { once: true });
    document.addEventListener('keydown', () => window.GameAudio?.unlockAudio?.(), { once: true });
