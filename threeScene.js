    ...
    wrap.innerHTML = `
      ...
      <div id="gw-lobby-confirm" class="gw-overlay gw-hide">
        <div class="gw-mbox">
          ...
          <button id="gw-lobby-no"  class="gw-abtn skp-btn">✕ No, Resume</button>
        </div>
      </div>
...
.gw-le.dice{border-color:var(--gold);color:var(--gold2);}
  ...
  function hookGameJS() {
    ...
    // Return Lobby button → show confirm modal
    ...
    // End Turn button → fire custom event
  ...
  function draw() {
    ...
    if (plannedPath.length > 0) {
      ...
      plannedPath.forEach((step, idx) => {
        ...
        ctx.globalAlpha = 0.9;
        ctx.strokeRect(step.col*CELL, step.row*CELL, CELL, CELL);
        ctx.restore();
