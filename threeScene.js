...
const ThreeScene = (() => {
  ...
  function buildUI() {
    ...
    ['gw-menu','gw-roll-btn','gw-confirm-btn','gw-endturn','gw-nextround','gw-lobby','gw-btn-attack','gw-btn-heal','gw-btn-skip','gw-restart'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.addEventListener('pointerdown', uiClickSfx);
    });
  }
...
.gw-le.dice{border-color:var(--gold);color:var(--gold2);}
  ...
  // ── Public API ────────────────────────────────────────────────────
  return {
    init: async (blocked) => { await init(blocked); },
    rebuildBoard,
